...
- `dependency_resolver/` — Resolves the dependency graph. `resolve_dependencies` + `toposort` produce an install order; `essential_dependencies`/`optional_dependencies` classify edges; `requirements` captures constraints; `advisor` consults `heuristics/` to pick arch-appropriate substitutes (e.g. BLAS → MKL/NVPL).
- `user_config_generator/` — Transforms the resolved dependency set into the user-editable YAML config. A chain of filters: `configurations_filter`, `options_filter`, `environment_filter`, `stages_filter`, unified by `config_transformer` and `user_config_generator`.
- `parser/` — Parses the *generated* user config back into executable commands. `user_config_parser` is the entry point; `source_commands` and `template_resolver` (with `parser_internal`) handle template/`uconf` expansion. (Implementation lives in `src/parser/`.)
- `ui/` — The interactive front end: `commands`/`ui` dispatch subcommands (`init`, `install`, `packages`, `environment`, `factory`, ...), `ui_utils` and `bash_completion` support them.
...
- `main.sh` — The `kez` shell function; forwards to the binary but intercepts commands that must affect the *current* shell (`env enter/exit`, `compiler load/unload`, `mpi load/unload`, `factory enter/exit`) and evaluates their output.
