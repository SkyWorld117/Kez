...
std::pair<YAML::Node, std::string> parse_run_config(const YAML::Node& factory_config,
    ...
    std::string launcher =
        multilevel_fetch(factory_config, cellar_config, profile_config, "launcher");
    if (launcher.empty()) {
        launcher = "none";
    }
    std::string launcher_opts =
        multilevel_fetch(factory_config, cellar_config, profile_config, "launcher_opts");

    std::string scheduler =
        multilevel_fetch(factory_config, cellar_config, profile_config, "scheduler");
    if (scheduler.empty()) {
        scheduler = "none";
    }
    std::string scheduler_opts =
        multilevel_fetch(factory_config, cellar_config, profile_config, "scheduler_opts");

    std::string num_nodes =
        profile_config["num_nodes"] ? profile_config["num_nodes"].as<std::string>() : "1";
    std::string num_procs_per_node = profile_config["num_procs_per_node"]
                                         ? profile_config["num_procs_per_node"].as<std::string>()
                                         : "1";
    std::string cores_per_proc =
        profile_config["cores_per_proc"] ? profile_config["cores_per_proc"].as<std::string>() : "1";
    std::string omp_num_threads = profile_config["omp_num_threads"]
                                      ? profile_config["omp_num_threads"].as<std::string>()
                                      : cores_per_proc;
    std::string gpus_per_proc =
        profile_config["gpus_per_proc"] ? profile_config["gpus_per_proc"].as<std::string>() : "0";

    std::string wrapped_command = wrap_with_resource_manager(
        target, launcher, launcher_opts, scheduler, scheduler_opts, num_nodes, num_procs_per_node,
        cores_per_proc, omp_num_threads, gpus_per_proc);
    run_instructions.push_back(wrapped_command);

    // Postrun commands
    std::string postrun =
        multilevel_fetch(factory_config, cellar_config, profile_config, "postrun");
    if (!postrun.empty()) {
    ...
    std::string summary_regex =
        multilevel_fetch(factory_config, cellar_config, profile_config, "summary_regex");
