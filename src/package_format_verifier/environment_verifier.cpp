...
bool verify_environment(const YAML::Node& node) {
    ...
    for (const auto& item : node) {
        ...
        if (item["user_configurable"] && !item["user_configurable"].as<bool>() && !item["default"] && !item["conditions"]) {
            WARNING("Warning at '" + std::to_string(item.Mark().line) + "':\nA non-user-configurable environment entry should have either a 'default' value or 'conditions', otherwise consider removing it.");
        }
    }
