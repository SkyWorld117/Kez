    ...
    for (const auto& item : node) {
        ...
        if (item["user_configurable"]) {
            if (!item["user_configurable"].IsScalar()) {
                ...
                return false;
            } else if (item["user_configurable"].as<std::string>() != "true" && item["user_configurable"].as<std::string>() != "false") {
                ERROR("Error at '" + std::to_string(item["user_configurable"].Mark().line) + "':\nA 'user_configurable' field, if present, must be either 'true' or 'false'.");
                return false;
            }
        }
        ...
        if (item["enabled"]) {
            ...
            if (item["enabled"]["default"]){
                if (!item["enabled"]["default"].IsScalar()) {
                    ...
                    return false;
                } else if (item["enabled"]["default"].as<std::string>() != "true" && item["enabled"]["default"].as<std::string>() != "false") {
                    ERROR("Error at '" + std::to_string(item["enabled"]["default"].Mark().line) + "':\nAn 'enabled' field's 'default' value, if present, must be either 'true' or 'false'.");
                    return false;
                }
            }
            if (item["enabled"]["conditions"] && !verify_conditions(item["enabled"]["conditions"])) {
        ...
        if (item["enabled_value"]) {
            ...
            if (item["enabled_value"]["default"] && !item["enabled_value"]["default"].IsScalar() && !item["enabled_value"]["default"].IsNull()) {
                ERROR("Error at '" + std::to_string(item["enabled_value"]["default"].Mark().line) + "':\nAn 'enabled_value' field's 'default' value, if present, must be a scalar or null.");
                return false;
            }
            if (item["enabled_value"]["conditions"] && !verify_conditions(item["enabled_value"]["conditions"])) {
                ERROR("Error at '" + std::to_string(item["enabled_value"]["conditions"].Mark().line) + "':\nInvalid conditions in 'enabled_value' field.");
                return false;
            }
            if (!item["enabled_value"]["default"] && !item["enabled_value"]["conditions"]) {
                WARNING("Warning at '" + std::to_string(item.Mark().line) + "':\nAn 'enabled_value' field should have either a 'default' value or 'conditions', otherwise consider removing it.");
            }
        }
        ...
        if (item["disabled_value"]) {
            ...
            if (item["disabled_value"]["default"] && !item["disabled_value"]["default"].IsScalar() && !item["disabled_value"]["default"].IsNull()) {
                ERROR("Error at '" + std::to_string(item["disabled_value"]["default"].Mark().line) + "':\nA 'disabled_value' field's 'default' value, if present, must be a scalar or null.");
                return false;
            }
            if (item["disabled_value"]["conditions"] && !verify_conditions(item["disabled_value"]["conditions"])) {
                ERROR("Error at '" + std::to_string(item["disabled_value"]["conditions"].Mark().line) + "':\nInvalid conditions in 'disabled_value' field.");
                return false;
            }
            if (!item["disabled_value"]["default"] && !item["disabled_value"]["conditions"]) {
                WARNING("Warning at '" + std::to_string(item.Mark().line) + "':\nA 'disabled_value' field should have either a 'default' value or 'conditions', otherwise consider removing it.");
            }
        }
    }
