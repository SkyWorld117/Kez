...
std::string parse_template(const std::string& template_str, ParserContext& context) {
    ...
    if (template_str.find("compiler.") == 0) {
        ...
        if (user_config_pkg["compiler"]) {
            ...
            return parse_property(compiler_name + "." + compiler_property, context);
        } else {
    ...
    if (std::find(abstract_packages.begin(), abstract_packages.end(), package_name) !=
        ...
        std::string result = parse_property(
            concrete_pkg_name + "." + template_str.substr(package_name.length() + 1), context);
        ...
        std::string result         = parse_property(template_str, context);
        template_map[template_str] = result;  // Cache the resolved template
        return result;
    }
}
