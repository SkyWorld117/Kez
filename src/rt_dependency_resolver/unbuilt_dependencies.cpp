...
std::vector<std::string> get_unbuilt_dependencies(
    ...
    preceding_packages.insert(
        preceding_packages.end(),
        std::find(user_packages.begin(), user_packages.end(), target_package) + 1,
        user_packages.end());
    ...
    for (const auto& dep : true_dependencies) {
        if (std::find(installed_packages.begin(), installed_packages.end(), dep) ==
