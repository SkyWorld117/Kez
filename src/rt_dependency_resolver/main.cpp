...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4) {
        ERROR("Usage: " + std::string(argv[0]) + " <user_config_file> <cellar_path> <target_package>");
        exit(EXIT_FAILURE);
    }
    ...
    std::vector<std::string> unbuilt_dependencies = get_unbuilt_dependencies(user_config, ins_yaml, installed_packages, target_package);
    packages_to_build.insert(packages_to_build.end(), unbuilt_dependencies.begin(), unbuilt_dependencies.end());
    // Get dependents
    std::vector<std::string> dependents = get_dependents(user_config, ins_yaml, installed_packages, target_package);
    ...
    packages_to_build.erase(std::unique(packages_to_build.begin(), packages_to_build.end()), packages_to_build.end());

    // Add target_package itself if not included
    if (std::find(packages_to_build.begin(), packages_to_build.end(), target_package) == packages_to_build.end()) {
    ...
    for (const auto& pkg_instruction : ins_yaml) {
        ...
        if (std::find(packages_to_build.begin(), packages_to_build.end(), pkg_name) != packages_to_build.end()) {
    ...
    std::filesystem::path filtered_ins_file = std::filesystem::path(cellar_path) / ".tmp/filtered_ins.yaml";
