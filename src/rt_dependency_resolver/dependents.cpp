...
std::vector<std::string> get_dependents(const YAML::Node& user_config, const YAML::Node& ins_yaml, const std::vector<std::string>& installed_packages, const std::string& target_package) {
    ...
    for (const auto& pkg : user_packages) {
        ...
        if (std::find(all_dependencies.begin(), all_dependencies.end(), target_package) != all_dependencies.end()) {
