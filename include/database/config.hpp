...
enum class Toolchain {
    ...
    Python,     ///< Python distribution installed into the shared virtual environment.
};
 ...
 * - For @c PyPI: releases contain distribution versions only; pip retrieves
 *   the selected distribution during shared environment synchronization.
 */
struct Source {
...
/**
 * @brief Concrete configuration for a Python distribution.
 *
 * Python-toolchain packages are first-class dependency nodes, but their
 * resolved distributions are installed together by the executor into the
 * target environment's shared virtual environment. They therefore provide no
 * per-package configure or build command defaults.
 */
class PythonPackageConfig final : public PackageConfig {
   public:
    Toolchain toolchain() const noexcept override;
};
